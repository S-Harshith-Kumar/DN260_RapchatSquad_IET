...
public class DBqueries {
    ...
    public static void removeFromWishlist(final int index, final Context context){
        ...
        Map<String,Object> updateWishist = new HashMap<>();
        for (int x = 0;x < wishList.size();x++){
            updateWishist.put("product_ID_"+x,wishList.get(x));
        }
        updateWishist.put("list_size",(long)wishList.size());
        firebaseFirestore.collection("USERS").document(FirebaseAuth.getInstance().getUid()).collection("USER_DATA").document("MY_WISHLIST")
                .set(updateWishist).addOnCompleteListener(new OnCompleteListener<Void>() {
            @Override
